...
版本 **1.1.0 / build 3**：新增航空公司、Squawk、中英文切换和可选飞机照片，保留定位修复与诊断。TestFlight 的 Mac 验证、签名归档和导出请先阅读 `MAC_HANDOFF.md`。当前包仍为源码，未签名、未归档、未上传。
...
- 航空公司：来自 adsbdb 的航空公司字段；常见航司中文译名，其余保留原名。缺失显示 N/A，不将飞机注册所有者直接当作实际承运人。
- 应答机代码 Squawk：保留四位及前导零；无效或缺失显示 N/A。不是六位 Mode-S 地址。
- 设置 → Language / 语言：简体中文与 English，选择保存在本机；界面、设置、诊断和实时活动标签同步切换。机场与航司专名可能保留原文，系统网络错误和系统权限弹窗语言由 iOS 控制。
- 飞机照片：按 Mode-S 地址通过 adsbdb 查询参考照片，Airport-Data.com 在线加载，并有来源与原图链接。无照片或加载失败显示占位，不影响航班刷新。设置中可关闭照片；不在演示模式加载真实照片。
- 航空公司 Logo 暂未接入：界面中的 NZ 等字母徽标为航司代码，不是官方 Logo。
- 照片查询只发送飞机地址；图片加载不发送用户坐标。照片不打包进工程；离线或第三方服务不可用时正常降级。

已核对实际航空公司、照片元数据及示例图片 HTTP 响应。Swift 语法 / 工程结构静态检查不能代替 Mac 上编译与真机测试；本次未执行 Apple SDK 构建和 XCTest。
